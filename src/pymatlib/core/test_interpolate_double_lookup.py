...
from pymatlib.core.interpolators import (interpolate_property, check_equidistant, temperature_from_energy_density,
                                         E_eq_from_E_neq, create_idx_mapping, prepare_interpolation_arrays, interpolate_double_lookup)
...
def create_alloy(T: Union[float, sp.Symbol]) -> Alloy:
    ...
    try:
        T_eq, E_neq, E_eq, idx_mapping = prepare_interpolation_arrays(
            alloy.temperature_array,
            alloy.energy_density_array
        )

        # Online execution
        E_target = 11789781961.76978
        start_time = time.perf_counter()  # More precise timing
        T_interpolate = interpolate_double_lookup(E_target, T_eq, E_neq, E_eq, idx_mapping)
    ...
    def measure_performance(iterations=1):
        ...
        for i in range(iterations):
            ...
            # results = [interpolate_double_lookup(E, T_eq, E_neq, E_eq, idx_mapping) for E in E_neq]
    ...
    measure_performance(iterations=int(1e5))
