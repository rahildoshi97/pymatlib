...
class PDFReporter:
    """Generates comprehensive PDF reports for material properties."""
    ...
    def _save_figure_safely(pdf: PdfPages, fig) -> None:
        """Save figure to PDF without layout warnings - compatible with all matplotlib versions."""
        try:
            # Check if set_tight_layout method exists (matplotlib 3.6+)
            ...
            # Save with standard parameters - no tight_layout manipulation needed
    ...
    def _create_single_property_page(self, pdf: PdfPages, prop_name: str,
        ...
        if config_lines > 20 or expr_lines > 15:
            ...
            self._create_single_page_property_layout(pdf, prop_name, prop_config, prop_type, config_text, expr_text)
    ...
    def _create_single_page_property_layout(self, pdf: PdfPages, prop_name: str,
        ...
        # Check if T is symbolic before attempting to plot - same as plotters.py
        if not isinstance(self.T_symbol, sp.Symbol):
            # No plot - use full space for config and expression
            ax_config = fig.add_axes([0.08, 0.60, 0.84, 0.25])
            ax_config.axis('off')
            self._add_config_section_manual(ax_config, config_text, prop_type)

            ax_expr = fig.add_axes([0.08, 0.25, 0.84, 0.25])
            ax_expr.axis('off')
            self._add_expression_section_manual(ax_expr, expr_text)

            # Add note about missing plot
            ax_note = fig.add_axes([0.08, 0.08, 0.84, 0.12])
            ax_note.axis('off')
            ax_note.text(0.5, 0.5, f"Plot not available (T is not symbolic: {type(self.T_symbol).__name__})",
    ...
    def _create_plot_only_page(self, pdf: PdfPages, prop_name: str,
        ...
        # Create subplot with PROPER positioning and aspect ratio
        ax = fig.add_axes([0.12, 0.25, 0.76, 0.55])

        # Check if T is symbolic before attempting to plot - same as plotters.py
        if not isinstance(self.T_symbol, sp.Symbol):
            ax.text(0.5, 0.5, f"Plot not available\n(T is not symbolic: {type(self.T_symbol).__name__})",
    ...
    def _create_plot_using_exact_plotters_logic(self, ax, prop_name: str, prop_config: Any, prop_type) -> None:
        ...
        Only generate plots if T is a symbolic variable, matching plotters.py behavior.
        """
        try:
            # Check if T is symbolic - same condition as plotters.py
            if not isinstance(self.T_symbol, sp.Symbol):
                logger.debug(f"Skipping plot for {prop_name} - T is not symbolic: {type(self.T_symbol)}")
                ax.text(0.5, 0.5, f"Plot not available\n(T is not symbolic: {type(self.T_symbol).__name__})",
                        ha='center', va='center', fontsize=10,
